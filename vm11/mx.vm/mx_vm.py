...
class BaseGraalVmLayoutDistribution(mx.LayoutDistribution):
    ...
    def __init__(self, suite, name, deps, components, is_graalvm, exclLibs, platformDependent, theLicense, testDistribution,
        ...
        if is_graalvm:
            ...
            if mx.get_os() == 'darwin':
                hsdis = '/lib/' + mx.add_lib_suffix('hsdis-' + mx.get_arch())
                incl_list, excl_list = _patch_darwin_jdk()
                for d, s in incl_list:
                    _add(layout, d, s)
                exclusion_list += excl_list
            else:
                hsdis = '/lib/' + mx.get_arch() + '/' + mx.add_lib_suffix('hsdis-' + mx.get_arch())
            ...
            if mx.get_os() == 'darwin':
                ...
                _add(layout, "<jdk_base>/lib/", "extracted-dependency:truffle:TRUFFLE_NFI_NATIVE/bin/<lib:trufflenfi>")
                _add(layout, "<jdk_base>/lib/server/vm.properties", "string:name=" + vm_name)
            else:
                _add(layout, "<jdk_base>/lib/<arch>/", "extracted-dependency:truffle:TRUFFLE_NFI_NATIVE/bin/<lib:trufflenfi>")
            ...
            if with_lib_polyglot:
                ...
                # Note that `lib/polyglot` is synchronized with `org.graalvm.polyglot.install_name_id` in `get_lib_polyglot_project`
                _add(layout, "<jdk_base>/lib/polyglot/" + lib_polyglot_project.native_image_name, "dependency:" + lib_polyglot_project.name)
                _add(layout, "<jdk_base>/lib/polyglot/", "dependency:" + lib_polyglot_project.name + "/*.h")
        ...
        for _component in self.components:
            ...
            if isinstance(_component, mx_sdk.GraalVmLanguage):
                _component_type_base = '<jdk_base>/languages/'
            elif isinstance(_component, mx_sdk.GraalVmTool):
                _component_type_base = '<jdk_base>/tools/'
                ...
                _component_type_base = '<jdk_base>/lib/'
                # Windows puts DLL's in <jdk_base>/bin
                if mx.get_os() == 'windows':
                    _component_jvmlib_base = '<jdk_base>/bin/'
                else:
                    _component_jvmlib_base = '<jdk_base>/lib/'
            ...
            _add(layout, '<jdk_base>/lib/boot/', ['dependency:' + d for d in _component.boot_jars], _component, with_sources=True)
            ...
            if isinstance(_component, mx_sdk.GraalVmJvmciComponent):
                _add(layout, '<jdk_base>/lib/jvmci/', ['dependency:' + d for d in _component.jvmci_jars], _component, with_sources=True)
            ...
            for _launcher_config in _get_launcher_configs(_component):
                _add(layout, '<jdk_base>/lib/graalvm/', ['dependency:' + d for d in _launcher_config.jar_distributions], _component, with_sources=True)
            ...
            for _library_config in _get_library_configs(_component):
                _add(layout, '<jdk_base>/lib/graalvm/', ['dependency:' + d for d in _library_config.jar_distributions], _component, with_sources=True)
                if not stage1:
        ...
        if has_graal_compiler:
            _add(layout, '<jdk_base>/lib/jvmci/compiler-name', 'string:graal')
...
class SvmSupport(object):
    ...
    def native_image(self, build_args, output_file, allow_server=False, nonZeroIsFatal=True, out=None, err=None):
        ...
        graal_sdk_jar = join(stage1.output, stage1.find_single_source_location('dependency:sdk:GRAAL_SDK'));
        truffle_api_jar = join(stage1.output, stage1.find_single_source_location('dependency:truffle:TRUFFLE_API'));
        graal_jar = join(stage1.output, stage1.find_single_source_location('dependency:compiler:GRAAL'));
        graal_management_jar = join(stage1.output, stage1.find_single_source_location('dependency:compiler:GRAAL_MANAGEMENT'));
        ...
        if provided_properties:
            ...
            if launcher_configs:
                ...
                properties = {
                    ...
                    'LauncherClassPath': graalvm_home_relative_classpath(launcher_config.jar_distributions, _get_graalvm_archive_path(None)),
...
""".format(workdir=join('languages', self.component.dir_name))
...
class GraalVmStandaloneComponent(mx.LayoutTARDistribution):  # pylint: disable=too-many-ancestors
    def __init__(self, installable, **kw_args):
        ...
        """
        support_dir_pattern = '<jdk_base>/languages/{}/'.format(installable.main_component.dir_name)
        other_comp_names = []
            ...
            if not has_polyglot_lib_entrypoints:
                ...
                lib_polyglot_config = mx_sdk.LibraryConfig(
                    ...
                    build_args=[
                        ...
                        "-Dorg.graalvm.polyglot.install_name_id=@rpath/lib/polyglot/<lib:polyglot>"
